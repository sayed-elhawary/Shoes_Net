    ...
    <div className="min-h-screen bg-[#18191a] text-white relative overflow-x-hidden">
      ...
      <AnimatePresence>
        {showCartModal && userRole === 'customer' && (
          ...
          >
            <motion.div
              className="bg-slate-900/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-slate-700 w-full max-w-lg"
            ...
            >
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {cart.map(item => (
                    <div
                      key={`${item.product._id}-${item.selectedImage}`}
                      className="flex items-center justify-between gap-4 p-3 bg-slate-800/50 rounded-xl"
                    >
                      <img
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="w-16 p-2 bg-slate-700 rounded text-center"
                        />
                        <button
              ...
              {cart.length > 0 && (
                <div className="flex gap-3 mt-6">
                  <button
                    onClick={() => { setShowCartModal(false); setShowOrderForm(true); }}
                    className="flex-1 py-3 rounded-xl bg-gradient-to-r from-red-600 to-red-700 text-white font-bold"
                  >
                    طلب
                  </button>
                  <button
                    onClick={() => setShowCartModal(false)}
                    className="flex-1 py-3 rounded-xl bg-gradient-to-r from-gray-600 to-gray-700 text-white font-bold"
                  >
                    إغلاق
                  </button>
                </div>
      ...
      <AnimatePresence>
        {showOrderForm && userRole === 'customer' && (
          ...
          >
            <motion.div
              className="bg-slate-900/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-slate-700 w-full max-w-md"
              onClick={e => e.stopPropagation()}
            >
              ...
              <input
                ...
                className="w-full p-4 mb-6 bg-slate-800/60 border border-slate-600 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/30"
              />
              <div className="flex gap-3">
                <button
                  onClick={handleOrderSubmit}
                  disabled={isSubmitting}
                  className="flex-1 py-3 rounded-xl bg-gradient-to-r from-red-600 to-red-700 text-white font-bold disabled:opacity-70"
                >
