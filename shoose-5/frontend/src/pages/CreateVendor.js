...
function CreateVendor() {
  ...
  const handleSubmit = () => {
    ...
    if (isEditing) {
      // تعديل تاجر موجود
      axios.put(`${process.env.REACT_APP_API_URL}/api/vendors/${editingId}`, form, {
        headers: { Authorization: `Bearer ${token}` },
      })
        .then(() => {
      ...
      // إنشاء تاجر جديد
      axios.post(`${process.env.REACT_APP_API_URL}/api/vendors`, form, {
        headers: { Authorization: `Bearer ${token}` },
      })
        .then(() => {
            ...
            >
              ...
              <div className="flex space-x-2 space-x-reverse">
                <motion.button
                  ...
                  className="w-full p-3 rounded-xl text-white font-semibold bg-blue-600/80 hover:bg-blue-600"
