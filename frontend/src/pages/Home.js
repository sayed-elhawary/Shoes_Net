...
const Home = () => {
  ...
  const fetchProducts = useCallback(() => {
    ...
    axios
      ...
      .then(res => {
        ...
        const initialIndexes = sortedProducts.reduce((acc, p) => ({ ...acc, [p._id]: 0 }), {});
        const initialTypes = sortedProducts.reduce((acc, p) => ({
          ...acc,
          [p._id]: p.videos && p.videos.length > 0 ? 'video' : 'image'
        }), {});
  ...
  const buttonVariants = { hover: { scale: 1.05, boxShadow: '0 8px 16px rgba(168, 85, 247, 0.3)' }, tap: { scale: 0.95 } };
    ...
    <div className="min-h-screen bg-[#18191a] text-white p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto">
        ...
        <motion.div className="flex flex-col items-center mb-8" initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          <h1 className="text-3xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-purple-600 mt-3">Web Shose</h1>
          <p className="text-purple-300 text-lg mt-1">المنتجات المتاحة</p>
        </motion.div>

        {/* الفلتر + السلة */}
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
          <motion.select value={filterType} onChange={e => setFilterType(e.target.value)} className="p-3 rounded-xl bg-slate-800/60 backdrop-blur-md border border-slate-600 text-white focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/30" whileHover={{ scale: 1.03 }}>
          ...
          <div className="flex items-center gap-2">
            <input type="number" placeholder="من (جوز)" value={priceRange.min} onChange={e => setPriceRange(prev => ({ ...prev, min: e.target.value }))} className="p-3 w-24 rounded-xl bg-slate-800/60 border border-slate-600 text-white placeholder:text-slate-400 focus:outline-none focus:border-purple-500" />
            <span className="text-slate-400">إلى</span>
            <input type="number" placeholder="إلى (جوز)" value={priceRange.max} onChange={e => setPriceRange(prev => ({ ...prev, max: e.target.value }))} className="p-3 w-24 rounded-xl bg-slate-800/60 border border-slate-600 text-white placeholder:text-slate-400 focus:outline-none focus:border-purple-500" />
          </div>
          {userRole === 'customer' && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.length === 0 ? (
            ...
            filteredProducts.map(product => (
              <motion.div key={product._id} className="bg-slate-900/60 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-700/50 overflow-hidden" variants={cardVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} whileHover="hover">
                <div className="relative aspect-square bg-slate-800">
                  {currentMediaType[product._id] === 'video' && product.videos?.length > 0 ? (
                    <video src={`${process.env.REACT_APP_API_URL}/Uploads/${product.videos[currentMediaIndex[product._id]]}`} controls className="w-full h-full object-contain" onClick={() => openMedia(product.videos[currentMediaIndex[product._id]], 'video')} />
                  ) : product.images?.length > 0 ? (
                    <img src={`${process.env.REACT_APP_API_URL}/Uploads/${product.images[(currentMediaIndex[product._id] || 0) - (product.videos?.length || 0)]}`} alt={product.name} className="w-full h-full object-contain" onClick={() => openMedia(product.images[(currentMediaIndex[product._id] || 0) - (product.videos?.length || 0)], 'image')} />
                  ) : (
                    <div className="w-full h-full bg-slate-700 flex items-center justify-center"><span className="text-slate-500">بدون صورة</span></div>
                  )}
                ...
                <div className="p-5 space-y-2 text-right">
                  ...
                  <div className="text-sm text-slate-300 space-y-1">
                    <p>التاجر: <Link to={`/vendors/${product.vendor?._id}/products`} className="text-purple-400 hover:text-purple-300">{product.vendor?.name}</Link></p>
                  ...
                  {userRole === 'customer' && (
                    <motion.button onClick={() => addToCart(product)} className="w-full mt-4 py-3 rounded-xl text-white font-bold bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 shadow-lg" variants={buttonVariants} whileHover="hover" whileTap="tap">إضافة إلى السلة</motion.button>
                  )}
                </div>
              </motion.div>
            ))
      ...
      <AnimatePresence>
        {showAddedToCart && (
          <motion.div className="fixed top-6 right-6 bg-purple-600 text-white px-5 py-3 rounded-full shadow-xl flex items-center gap-2 z-50" variants={toastVariants} initial="hidden" animate="visible" exit="hidden">
            ...
            <motion.div className="bg-slate-900/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-slate-700 w-full max-w-lg" onClick={e => e.stopPropagation()}>
                  ...
                  {cart.map(item => (
                    <div key={`${item.product._id}-${item.selectedImage}`} className="flex items-center justify-between gap-4 p-3 bg-slate-800/50 rounded-xl">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => removeFromCart(item.product._id, item.selectedImage)} className="text-purple-400 hover:text-purple-300">×</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {cart.length > 0 && (
                <div className="flex gap-3 mt-6">
                  <button onClick={() => { setShowCartModal(false); setShowOrderForm(true); }} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-purple-700 text-white font-bold">طلب</button>
        ...
        {showOrderForm && userRole === 'customer' && (
          <motion.div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50" variants={modalVariants} initial="hidden" animate="visible" exit="hidden" onClick={() => setShowOrderForm(false)}>
            <motion.div className="bg-slate-900/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-slate-700 w-full max-w-md" onClick={e => e.stopPropagation()}>
              ...
              <input type="text" placeholder="العنوان" value={orderForm.address} onChange={e => setOrderForm({ address: e.target.value })} className="w-full p-4 mb-6 bg-slate-800/60 border border-slate-600 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/30" />
              <div className="flex gap-3">
                <button onClick={handleOrderSubmit} disabled={isSubmitting} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-purple-700 text-white font-bold disabled:opacity-70">
        ...
        {selectedMedia && (
          <motion.div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50" variants={modalVariants} initial="hidden" animate="visible" exit="hidden" onClick={closeMedia}>
            <motion.div className="relative max-w-4xl max-h-screen p-4" onClick={e => e.stopPropagation()}>
              ...
              <button onClick={closeMedia} className="absolute top-4 right-4 bg-purple-600 text-white w-10 h-10 rounded-full flex items-center justify-center text-xl shadow-lg hover:bg-purple-700">×</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Messages Modal (مطابق تمامًا لـ Orders.js) */}
      <AnimatePresence>
